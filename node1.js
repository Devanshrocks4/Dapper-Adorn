...
const mongoURI = 'mongodb+srv://root:root@cluster0.gmwwxjz.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0';
...
//   console.log("hi")
//   fs.writeFile('request.log', `${new Date()} +'\n' + ${req.url}+'\n'`, (err) => {
//       if (err) {
//         console.error(err); 
//         return;
//       }});
...
//   });
app.use((req,res,next) =>{
  fs.appendFile('request.log', new Date() +'\n' + req.url +'\n', (err) => {
    if (err) {
      console.error(err); 
      return;
    }});
...
app.post('/log', async (req, res) => {
    ...
    try {
        // Look up the user using await with a promise
